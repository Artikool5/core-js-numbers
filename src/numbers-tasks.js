...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const getVectorProduct = (xa, xb, ya, yb) => xa * xb + ya * yb;
  const getVectorLength = (x, y) => Math.sqrt(x ** 2 + y ** 2);
  const angle =
    getVectorProduct(x1, x2, y1, y2) /
    (getVectorLength(x1, y1) * getVectorLength(x2, y2));
...
function toNumber(value, def) {
  ...
  if (typeof value === 'string') {
    ...
    return Number.isNaN(parsedNumber) ? def : parsedNumber;
  }
  return value;
}
...
function getSumOfDigits(num) {
  ...
  let sum = 0;
  let internalNum = num;
  for (let i = 1; internalNum / 10 > 0; i += 1) {
    sum += internalNum % 10;
    internalNum = Math.trunc(internalNum / 10);
  }
  return sum;
}
...
function isPowerOfTwo(num) {
  let internalNum = num;
  while (internalNum >= 2) {
    if (internalNum % 2 !== 0) return false;
    internalNum /= 2;
  }
  return true;
}
...
function getRandomInteger(min, max) {
  const temp = min + Math.trunc(Math.random() * 10);
...
function getCountOfOddNumbers(number) {
  const trunced = Math.abs(Math.trunc(number / 2));
  return number % 2 !== 0 ? trunced + 1 : trunced;
}
